...
func NewServer(bc *inner.Bitcask, addr string) *Server {
	return &Server{
		bc:        bc,
		addr:      addr,
...
func (s *Server) handleCommand(conn redcon.Conn, cmd redcon.Command) {
	...
	// 检查键是否过期 (对于需要的命令)
	if command != "PING" && command != "INFO" && command != "KEYS" && command != "TTL" && command != "QUIT" && command != "EXPIRE" {
		if len(cmd.Args) > 1 {
			s.checkAndRemoveExpired(string(cmd.Args[1]))
		}
	}
...
// 检查并移除过期键
func (s *Server) checkAndRemoveExpired(key string) {
	ttlKey := encodeKeyExpire(key)
	ttlBytes, err := s.bc.Get([]byte(ttlKey))
	if err != nil {
		return // 键不存在或无过期时间
	}
	...
	if isExpired(ttlBytes) {
		// 键已过期，删除相关数据
		keyTypeBytes, err := s.bc.Get([]byte(encodeKeyType(key)))
		if err == nil {
...
func (s *Server) handleGet(conn redcon.Conn, key []byte) {
	// 检查键类型
	keyTypeBytes, err := s.bc.Get([]byte(encodeKeyType(string(key))))
	if err == nil {
		keyType := string(keyTypeBytes)
...
func (s *Server) handleKeys(conn redcon.Conn, pattern []byte) {
	...
	err := s.bc.Scan(func(key []byte, _ []byte) error {
		...
		if !seen[keyStr] {
			// 检查是否过期
			ttlBytes, err := s.bc.Get([]byte(encodeKeyExpire(keyStr)))
			if err == nil && isExpired(ttlBytes) {
				// 键已过期，不包含在结果中
				return nil
			}
...
func (s *Server) handleExpire(conn redcon.Conn, key, seconds []byte) {
	...
	_, err := s.bc.Get([]byte(encodeKeyType(keyStr)))
	if err == nil {
		exists = true
	} else {
		// 检查是否是原始字符串键
...
func (s *Server) handleTTL(conn redcon.Conn, key []byte) {
	...
	conn.WriteInt(int(ttl)) // 修复: 将int64转换为int
}
