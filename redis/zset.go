...
func (s *Server) handleZAdd(conn redcon.Conn, key []byte, args [][]byte) {
	...
	for i := 0; i < len(args); i += 2 {
		...
		// 检查成员是否已存在
		oldScoreBytes, err := s.bc.Get([]byte(encodeZSetScoreKey(keyStr, member)))
		if err == nil {
			// 成员已存在，更新分数
...
func (s *Server) handleZRange(conn redcon.Conn, args [][]byte) {
	...
	// 检查键类型
	keyTypeBytes, err := s.bc.Get([]byte(encodeKeyType(keyStr)))
	if err != nil || string(keyTypeBytes) != TypeZSet {
		conn.WriteArray(0)
		return
	}
...
func (s *Server) handleZRank(conn redcon.Conn, key []byte, member []byte) {
	...
	// 获取成员的分数
	_, err = s.bc.Get([]byte(encodeZSetScoreKey(keyStr, memberStr)))
	if err != nil {
...
func (s *Server) handleZScore(conn redcon.Conn, key []byte, member []byte) {
	...
	// 获取成员的分数
	scoreBytes, err := s.bc.Get([]byte(encodeZSetScoreKey(keyStr, memberStr)))
	if err != nil {
		conn.WriteNull() // 成员不存在
		return
	}
