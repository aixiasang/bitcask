...
func (s *Server) handleHSet(conn redcon.Conn, key []byte, args [][]byte) {
	...
	for i := 0; i < len(args); i += 2 {
		...
		// 检查字段是否已存在
		_, err := s.bc.Get([]byte(encodeHashKey(keyStr, field)))
		isNew := err != nil
...
func (s *Server) handleHGet(conn redcon.Conn, key []byte, field []byte) {
	...
	// 检查键类型
	keyTypeBytes, err := s.bc.Get([]byte(encodeKeyType(keyStr)))
	if err != nil || string(keyTypeBytes) != TypeHash {
		conn.WriteNull()
		return
	}

	// 获取哈希字段值
	value, err := s.bc.Get([]byte(encodeHashKey(keyStr, fieldStr)))
	if err != nil {
		conn.WriteNull()
		return
	}
...
func (s *Server) handleHExists(conn redcon.Conn, key []byte, field []byte) {
	...
	// 检查字段是否存在
	_, err = s.bc.Get([]byte(encodeHashKey(keyStr, fieldStr)))
	if err != nil {
		conn.WriteInt(0)
	} else {
		conn.WriteInt(1)
