...
func (s *Server) handleLPush(conn redcon.Conn, key []byte, values [][]byte) {
	...
	// 检查键类型
	keyTypeBytes, err := s.bc.Get([]byte(encodeKeyType(keyStr)))
	if err == nil {
		// 键已存在，检查类型
	...
	for i := len(values) - 1; i >= 0; i-- {
		...
		for j := length; j > 0; j-- {
			// 移动现有元素
			oldValue, err := s.bc.Get([]byte(encodeListKey(keyStr, j-1)))
			if err == nil {
...
func (s *Server) handleLPop(conn redcon.Conn, key []byte) {
	...
	// 检查键类型
	keyTypeBytes, err := s.bc.Get([]byte(encodeKeyType(keyStr)))
	if err != nil || string(keyTypeBytes) != TypeList {
		conn.WriteNull()
		return
	}
	...
	// 获取第一个元素
	value, err := s.bc.Get([]byte(encodeListKey(keyStr, 0)))
	if err != nil {
		conn.WriteNull()
		return
	}
	...
	for i := 0; i < length-1; i++ {
		nextValue, err := s.bc.Get([]byte(encodeListKey(keyStr, i+1)))
		if err == nil {
...
func (s *Server) handleRPop(conn redcon.Conn, key []byte) {
	...
	// 获取最后一个元素
	lastIndex := length - 1
	value, err := s.bc.Get([]byte(encodeListKey(keyStr, lastIndex)))
	if err != nil {
		conn.WriteNull()
		return
	}
...
func (s *Server) handleLRange(conn redcon.Conn, key, start, stop []byte) {
	...
	for i := startIdx; i <= stopIdx; i++ {
		value, err := s.bc.Get([]byte(encodeListKey(keyStr, i)))
		if err == nil {
